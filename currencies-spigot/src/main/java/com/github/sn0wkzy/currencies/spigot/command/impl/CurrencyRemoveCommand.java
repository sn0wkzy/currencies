...
public class CurrencyRemoveCommand extends CustomCommand {

    private final Currency currency;
    private final CurrencyUserCache currencyUserCache;

    public CurrencyRemoveCommand(Currency currency, CurrencyUserCache currencyUserCache) {
        super("remover", "currencies.remove", false, "remove");
    ...
    protected void onCommand(CommandSender commandSender, String[] arguments) {
        if (arguments.length < 2) {
            commandSender.sendMessage("§cUtilize /" + currency.getSingular() + " remover <jogador> <quantia>");
            return;
        }
        ...
        if (target.isOnline()) {
            final String message = ConfigurationValue.get(ConfigurationValue::CURRENCY_REMOVE_RECEIVE)
        ...
        final String message = ConfigurationValue.get(ConfigurationValue::CURRENCY_REMOVE_SEND)
                .replace("{amount}", NumberFormatter.formatWithSuffix(amount))
                .replace("{currency_color}", currency.getColor())
                .replace("{currency_symbol}", currency.getSymbol())
